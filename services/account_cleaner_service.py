...
from config import TELEGRAM_API_ID, TELEGRAM_API_HASH, TELEGRAM_PHONE, BASE_DIR
from utils.logger import logger

SESSION_FILE = str(BASE_DIR / "telegram_user_session")


class AccountCleanerService:
    """Telegram hisobini tozalash (Deleted Accounts, eskirgan kanallar va dialoglar) xizmati."""
    ...
    async def get_client():
        """Telethon mijozini asinxron tarzda qaytaradi."""
        ...
        if AccountCleanerService._client is None:
            AccountCleanerService._client = TelegramClient(
                SESSION_FILE,
                int(TELEGRAM_API_ID),
                TELEGRAM_API_HASH
            )

        if not AccountCleanerService._client.is_connected():
            await AccountCleanerService._client.connect()

        return AccountCleanerService._client
    ...
    async def is_authorized() -> bool:
        ...
        if not AccountCleanerService.is_configured():
            return False
        try:
            client = await AccountCleanerService.get_client()
            return await client.is_user_authorized()
        except Exception as e:
            logger.warning(f"Avtorizatsiyani tekshirishda xatolik: {e}")
            return False

    @staticmethod
    async def send_auth_code(phone: str) -> str:
        """Telefon raqamga Telegram tasdiqlash kodini yuboradi."""
        client = await AccountCleanerService.get_client()
        result = await client.send_code_request(phone)
        AccountCleanerService._phone_code_hash = result.phone_code_hash
        return result.phone_code_hash

    @staticmethod
    async def complete_sign_in(phone: str, code: str, password: Optional[str] = None) -> tuple[bool, str]:
        """Tasdiqlash kodi va (agar bor bo'lsa 2FA parol) orqali tizimga kirish."""
        client = await AccountCleanerService.get_client()
        from telethon.errors import SessionPasswordNeededError

        try:
            await client.sign_in(phone=phone, code=code, phone_code_hash=AccountCleanerService._phone_code_hash)
    ...
    @staticmethod
    async def scan_deleted_accounts() -> list[dict]:
    ...
    @staticmethod
    async def remove_deleted_accounts() -> int:
        """Barcha 'Deleted Account' bo'lib qolgan chatlarni o'chiradi."""
        client = await AccountCleanerService.get_client()
        deleted_list = await AccountCleanerService.scan_deleted_accounts()
        count = 0
    ...
    @staticmethod
    async def scan_inactive_channels(days: int = 60) -> list[dict]:
        ...
        for d in dialogs:
            if d.is_channel or d.is_group:
                # Agar oxirgi xabar cutoff_date dan eski bo'lsa
                dialog_date = d.date
    ...
    @staticmethod
    async def leave_inactive_channels(channel_ids: list[int]) -> int:
    ...
    @staticmethod
    async def scan_old_dialogs(days: int = 90) -> list[dict]:
    ...
    @staticmethod
    async def delete_old_dialogs(dialog_ids: list[int]) -> int:
